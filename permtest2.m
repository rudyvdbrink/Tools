...
if rv %if we're only comparing the mean of vectors
    ...
    for permi = 1:npermutes
        bnull = [a; b];
        ind = randperm(length(bnull),length(a));
        anull = bnull(ind);
        bnull(ind) = [];
...
%if we're comparing the mean across a particular dimension of a matrix
else %if we're comparing the mean across a particular dimension of a matrix
    ...
    for permi = 1:npermutes     
        idx = randperm(size(cnull,1),size(a,dim));        
        anull = cnull;
        bnull = cnull;
        anull(setdiff(1:size(cnull,1),idx),:) = [];
        bnull(idx,:)  = [];
